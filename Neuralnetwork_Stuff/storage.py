...
class Storage:
    ...
    def reward(self):
        ...
        current_state= self.game.return_state()
        reward+=self.did_an_action_do_something(current_state)
        self.all_states.append(current_state)
        return reward


    #first only this function to test if the nn can atleast do leagal moves
    def did_an_action_do_something(self,current_state):
        if not self.all_states:
            return 0
        if current_state == self.all_states[-1]:
            return -1
        else:
            return 1
