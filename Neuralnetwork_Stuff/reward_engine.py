...
class RewardEngine():
    ...
    def reward(self):
        ...
        if not self.did_an_action_do_something():reward+=0#; print("action did something")
        if self.did_an_action_do_something(): reward += 2
        if self.punish_back_and_forth(): reward-=0.2#; print("back and forth punished")
        if self.put_card_in_the_middel(): reward+=5#; print("card in the middel")
        if self.card_fromplayer_toboard(): reward+=2 #; print("card from player to board")
        if self.did_end_move(): reward+=1#; print("did end move")
        if self.count_additional_spaces(): reward += self.count_additional_spaces()*1.15#;print("created space")
