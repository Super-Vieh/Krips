...
class SpielInitialisierer:
    ...
    def play_init(game: Spiel):
        ...
        while game.gameon:
            ...
            while game.current.anderreihe == True:
                ...
                SpielInitialisierer.print_top(game)
                SpielInitialisierer.print_sidesplus(game)
                SpielInitialisierer.print_bot(game)
    ...
    def play_console(game: Spiel, db: 'Datenbank'):
        ...
        ergtuple = SpielInitialisierer.convertiere_spielkartenstand(game)
    ...
    def play_from_db(game: Spiel, db: 'Datenbank', whatgamenr: int):
        SpielInitialisierer.clear_game_lists(game)
        # hier wird der Anfangskartenstand aus der Datenbank geladen
        SpielInitialisierer.convertiere_spielkartenstand_aus_db(db.extrahiere_spielkarten(whatgamenr), game)
        ...
        while game.gameon:
            ...
            while game.current.anderreihe == True:
                ...
                SpielInitialisierer.print_sidesplus(game)
                SpielInitialisierer.print_bot(game)
                (action, actions) = SpielInitialisierer.take_first_action(actions)
    ...
    def convertiere_spielkartenstand_aus_db(allekarten: tuple[tuple[int, str, str, str]], game: Spiel):
        for zeile in allekarten:
            (aktion, rest) = ("", zeile[1])
            while (aktion, rest) != ("M", ""):
                aktion, rest = SpielInitialisierer.take_first_action(rest)
                if aktion == "M": break
                SpielInitialisierer.fuege_karte_zueiner_liste_hinzu(SpielInitialisierer.stringzukarte(aktion), zeile[0], "Paeckchen", game)
            (aktion, rest) = ("", zeile[2])
            while (aktion, rest) != ("M", ""):
                (aktion, rest) = SpielInitialisierer.take_first_action(rest)  # kann "M" zurückgeben
                if aktion == "M": break
                SpielInitialisierer.fuege_karte_zueiner_liste_hinzu(SpielInitialisierer.stringzukarte(aktion), zeile[0], "Dreizehner", game)
            (aktion, rest) = ("", zeile[3])
            while (aktion, rest) != ("M", ""):
                (aktion, rest) = SpielInitialisierer.take_first_action(rest)  # kann "M" zurückgeben
                if aktion == "M": break
                SpielInitialisierer.fuege_karte_zueiner_liste_hinzu(SpielInitialisierer.stringzukarte(aktion), zeile[0], "4Karten", game)
    ...
    def print_top(game: Spiel):  # Folgende Print ausgaben waren mit "!Hilfe" von Chat GPT erstellt
        ...
        try:
            sp2pl = game.spieler2Paechen[len(game.spieler2Paechen) - 1]
        except IndexError:
            sp2pl = None

        try:
            sp2hl = game.spieler2Haufen[len(game.spieler2Haufen) - 1]
        except IndexError:
            sp2hl = None

        try:
            sp2dl = game.spieler2Dreizehner[len(game.spieler2Dreizehner) - 1]
        except IndexError:
            sp2dl = None
    ...
    def print_bot(game: Spiel):
        ...
        # Es kann passieren das die listen kein ellement haben dann wird null geprintet
        try:
            sp1pl = game.spieler1Paechen[- 1]
        except IndexError:
            sp1pl = None

        try:
            sp1hl = game.spieler1Haufen[len(game.spieler1Haufen) - 1]
        except IndexError:
            sp1hl = None

        try:
            sp1dl = game.spieler1Dreizehner[- 1]
        except IndexError:
            sp1dl = None
    ...
    def print_sidesplus(game: Spiel):
        for i in range(4):
            if i == 0:
                ...
                SpielInitialisierer.print_middle(game, i)
                for j in range(pl5l):
