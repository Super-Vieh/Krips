...
def fuege_karte_der_gamelist_hinzu(self, card: Card, x_wert: int, y_wert: int, bild: pygame.Surface | None = None) -> None:
        if not card.is_face_up:
            ...
            dargestelltes_bild: pygame.Surface = geholtes_bild_liste[card.card_rank.value - 1]
...
def erstelle_centerlist(self, bild: pygame.Surface) -> None:
    ...
    for i in range(8):
        ...
        if current_list:
            ...
            geholtes_bild: pygame.Surface = kartendarstellung[current_list[-1].card_rank.card_rank - 1]
        if i % 2 == 0:
            if not current_list:
...
def erstelle_sidelist(self) -> None:
    for i in range(8):
        ...
        for k in self.game.tableau[i]:
            ...
            geholtes_bild: pygame.Surface = kartendarstellung[k.card_rank.card_rank - 1]
